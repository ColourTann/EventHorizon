...
public class Test extends Screen{
	Sink camera=new Sink(Main.width/2,Main.height/2);
	@Override
	public void update(float delta) {
		...
		camera=camera.add(new Sink(x,y));
	}
	...
	public void render(SpriteBatch batch) {
		...
		Junk.drawTextureRotatedScaledCentered(batch, Gallery.shipAurora.get(), (int)camera.x, (int)camera.y,.2f,.2f, new Sink(Main.width/2,Main.height/2).getAngle(camera));
